...
public final class ModelElements {
    ...
    public static void applyAnimatedModels(ResourceManager resourceManager) {
        Map<EntityType<?>, EntityRenderer<?, ?>> entityRenderers = Minecraft.getInstance()
                .getEntityRenderDispatcher().renderers;
        EntityRendererProvider.Context context = getEntitiesContext(resourceManager);
        for (Map.Entry<EntityType<?>, EntityRenderer<?, ?>> entry : entityRenderers.entrySet()) {
            if (!BetterAnimationsCollection.CONFIG.get(ClientConfig.class).mobBlacklist.contains(entry.getKey())
                    && entry.getValue() instanceof LivingEntityRenderer<?, ?, ?> entityRenderer
                    && entityRenderer.getModel() != null) {
                for (Map.Entry<Identifier, ModelElement> modelElement : MODEL_ELEMENTS.entrySet()) {
                    try {
                        modelElement.getValue().onApplyModelAnimations(entityRenderer, context);
