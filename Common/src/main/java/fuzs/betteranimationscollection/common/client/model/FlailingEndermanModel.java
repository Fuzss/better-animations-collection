...
import fuzs.betteranimationscollection.common.client.element.FlailingEndermanElement;
...
public class FlailingEndermanModel extends EndermanModel<EndermanRenderState> {
    ...
    public void setupAnim(EndermanRenderState state) {
        ...
        if (state.isCreepy && (state.carriedBlock.isEmpty() || FlailingEndermanElement.whileCarrying)) {
            ...
            if (state.carriedBlock.isEmpty()) {
                ...
                for (int i = 0; i < ENDERMAN_ARM_LENGTH; i++) {
                    float armPartZRot =
                            Mth.sin(state.ageInTicks * animationSpeed * 7 + (float) i * 0.45F) * ((float) (i + 8)
                                    / 8.0F) * animationSpeed;
                ...
                for (int i = 0; i < ENDERMAN_ARM_LENGTH; i++) {
                    ...
                    float armPartZRot =
                            Mth.sin(state.ageInTicks * animationSpeed * 5 + (float) j * 0.45F) * ((float) (j + 8)
                                    / 8.0F) * animationSpeed;
