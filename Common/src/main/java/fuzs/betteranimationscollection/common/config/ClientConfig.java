package fuzs.betteranimationscollection.common.config;
...
public class ClientConfig implements ConfigCore {
    ...
    public void addToBuilder(ModConfigSpec.Builder builder, ValueCallback callback) {
        ...
        for (Map.Entry<Identifier, ModelElement> entry : ModelElements.getModelElements().entrySet()) {
            ModelElement modelElement = entry.getValue();
            callback.accept(builder.comment(modelElement.getDescriptionComponent())
                    .define(entry.getKey().getPath(), modelElement.getDefaultState()), modelElement::setEnabled);
        }
